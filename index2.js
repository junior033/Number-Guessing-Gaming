...
function generateCard(){
  ...
  let name = '[' + name1[getRandomInt(0, name1.length)] + ' ' + name2[getRandomInt(0, name2.length)] + ' AP: ' + getRandomInt() + ']';
  return name;

}

//adding cards to user hand array
...
function drawPhase()
{
    ...
    let draw = prompt.question('Would you like to draw a card? y/n'); 
    if(draw === 'y')
    {
...
{ 
  if(battlefieldUser === undefined)//checking if element in index(card in field)
  {
    ...
    if (returnCard === 'y')
    {
      ...
      let index = prompt.questionInt('Enter 0-3 to play: ' + userHand + '\n');
      ...
      console.log('You sent '+ battlefieldUser + ' to the battlefield!')
        ...
        let playCard = prompt.question('Enter 0-3 to play: ' + userHand + '\n');       
  ...
  if(endmainPhase === 'y')
  {
    //Finding int in string for battlephase
    let userStr = battlefieldUser[0];
    userStr = userStr[0].split('');
    let userAPstr = userStr.find(element => element > 0);
    userAP = parseInt(userAPstr);
    console.log('   ***Entering Battlephase***');battlePhase()
  }else{endofUserTurn()}
}
...
//end turn phase function stops game when comp or user reaches 3 wins
function endofUserTurn(){
 ...
 //computer end turn phase function stops game when comp or user reaches 3 wins
...
function compMainPhase()
{
  ...
  let compStr;
  compStr = compDeck[iComp].split('');  
  let compAPstr = compStr.find(element => element > 0);
